...
class KotakNeoBroker(BrokerInterface):
    ...
    def __init__(self):
        ...
        # Kotak Neo TOTP endpoint expects the Indian registered mobile in
        # country-code form. Normalize common inputs to 91XXXXXXXXXX.
        raw_mobile = os.environ.get("KOTAK_MOBILE", "")
        mobile_digits = re.sub(r"\D", "", raw_mobile)
        if mobile_digits.startswith("0") and len(mobile_digits) == 11:
            mobile_digits = mobile_digits[1:]
        if mobile_digits.startswith("91") and len(mobile_digits) == 12:
            self.mobile = mobile_digits
        elif len(mobile_digits) == 10 and mobile_digits[0] in "6789":
        ...
        if self.mobile and (len(self.mobile) != 12 or not self.mobile.startswith("91")):
            self._mobile_format_error = "KOTAK_MOBILE must be an Indian mobile number; normalized format is 91XXXXXXXXXX"
        else:
            self._mobile_format_error = None
    ...
    def authenticate(self) -> bool:
        ...
        with self._lock:
            ...
            try:
                ...
                logger.info("[KOTAK] TOTP request mobile format: %d digits, prefix=%s, last4=%s.",
                            len(self.mobile or ""), (self.mobile or "")[:2],
                            (self.mobile or "")[-4:] if self.mobile else "----")
